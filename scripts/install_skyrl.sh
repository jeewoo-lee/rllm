#!/bin/bash
# Adapted from install_verl.sh for skyrl installation
...
pip install "transformers[hf_xet]>=4.51.0" accelerate peft hf-transfer \
    "numpy<2.0.0" "pyarrow>=19.0.1" pandas \
    "ray==2.51.1" codetiming hydra-core pylatexenc qwen-vl-utils wandb dill pybind11 liger-kernel mathruler blobfile xgrammar \
    pytest py-spy pyext pre-commit ruff \
    "datasets==4.0.0" debugpy==1.8.0 func_timeout jaxtyping polars s3fs tensorboard
...
echo "5. Install skyrl"
if [ ! -d "skyrl/skyrl-train" ]; then
    git submodule update --init --recursive skyrl || true
fi
if [ ! -d "skyrl/skyrl-train" ]; then
    echo "Error: skyrl/skyrl-train directory not found. Please ensure the skyrl submodule is properly initialized."
    exit 1
fi
pip install --no-deps -e skyrl/skyrl-train/
